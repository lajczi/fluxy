    ...
    try {
      ...
      switch (subcommand) {
        case 'add': {
          ...
          if (settings.commandAllowedRoles?.includes(roleId)) {
            return void (await message.reply(
              t(lang, 'commands.admin.commandperms.alreadyAllowed', { roleName: role.name }),
            ));
          }
        ...
        case 'remove': {
          ...
          if (!roleArg) {
            return void (await message.reply(
              t(lang, 'commands.admin.commandperms.roleRequiredUsageRemove', { prefix }),
            ));
          }
          ...
          const note =
            settings.commandAllowedRoles.length === 0 ? t(lang, 'commands.admin.commandperms.noteAllowlistEmpty') : '';
