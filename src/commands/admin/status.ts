...
const command: Command = {
  ...
  async execute(message, args, client) {
    ...
    try {
      ...
      const automodStatus =
        am.level && am.level !== 'off'
          ? t(lang, 'commands.admin.status.enabled', { value: am.level })
          : t(lang, 'commands.admin.status.disabled');
      const antiLinkStatus = am.antiLink
        ? t(lang, 'commands.admin.status.enabledSimple')
        : t(lang, 'commands.admin.status.disabledSimple');
      const antiSpamStatus = am.antiSpam
        ? t(lang, 'commands.admin.status.enabledSimple')
        : t(lang, 'commands.admin.status.disabledSimple');
      const antiReactionSpamStatus = am.antiReactionSpam
        ? t(lang, 'commands.admin.status.enabledSimple')
        : t(lang, 'commands.admin.status.disabledSimple');
      const antiGhostPingStatus = am.ghostPing
        ? t(lang, 'commands.admin.status.enabledSimple')
        : t(lang, 'commands.admin.status.disabledSimple');
      ...
      const logChannel = modLogChannelId ? `<#${modLogChannelId}>` : t(lang, 'commands.admin.status.logNotSet');
      ...
      const muteRole = settings.muteRoleId ? `<@&${settings.muteRoleId}>` : t(lang, 'commands.admin.status.muteNotSet');
      ...
      embed.addFields({
        ...
        value: settings.prefix
          ? `\`${prefix}\``
          : t(lang, 'commands.admin.status.customPrefixDefaultValue', { defaultPrefix }),
        inline: true,
      });
      ...
      if (isNetworkError(error)) {
        console.warn(
          `[${(message as any).guild?.name || 'Unknown Server'}] Fluxer API unreachable during !status (ECONNRESET)`,
        );
      } else {
